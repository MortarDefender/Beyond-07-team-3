...
@pytest.fixture
def new_event():
    return Event(title=TITLE, location=LOCATION, description=DESCRIPTION,
...
@pytest.mark.parametrize('title, date_time_start, date_time_end, expected_error', [
    ...
    (TITLE, None, DATE_TIME_END, 'date time fileds should not be left empty'),
    (TITLE, DATE_TIME_START, None, 'date time fileds should not be left empty'),
    (TITLE, DATE_TIME_START, DATE_TIME_START, f'the statring date time should be greater then the ending date time'),
    (TITLE, DATE_TIME_END, DATE_TIME_START, f'the statring date time should be greater then the ending date time')
])
